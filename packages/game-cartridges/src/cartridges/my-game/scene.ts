...
export function createStarterScene(context: CartridgeGameConfigContext): typeof Phaser.Scene {
  return class StarterScene extends Phaser.Scene {
    create(this: Phaser.Scene) {
      ...
      if (gameConfig.scene) {
        const scenesToRegister = Array.isArray(gameConfig.scene) ? gameConfig.scene : [gameConfig.scene];
        ...
        if (firstScene && typeof firstScene === 'function') {
          ...
          if (firstSceneName) {
            this.scene.start(firstSceneName, { contestContext: context }); // ส่งข้อมูลระบบประกวดพ่วงไปด้วย
          }
        }
      }

      
      context.diagnostic({ code: "GAME_READY", message: "Survival game cartridge is ready" });
    }
  };
}
